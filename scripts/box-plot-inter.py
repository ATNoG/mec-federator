...
def process_alert_phases(alerts):
    """
    Process alerts from a single run to extract phase durations.
...
def plot_results(sent_total, phase_durations, output=None):
    """
    Function to create a box plot showing migration phase durations.
    ...
    logging.info(f"\nMigration Phase Duration Statistics (from {sent_total} total messages):")
    for phase_name, durations in phase_durations.items():
        if durations:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot migration phase durations from multiple runs in a folder")
    parser.add_argument("-d", "--folder", help="Path to the folder containing JSON result files", default="results/")
    parser.add_argument("-o", "--output", help="Output file to save plot (default: results/migration_phases.pdf)")
